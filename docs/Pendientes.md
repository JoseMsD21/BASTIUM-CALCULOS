...
- [Sprint 24 — Validación de datos: formularios de obligaciones y parámetros legales versionados ✅ Completado](#sprint-24--validación-de-datos-formularios-de-obligaciones-y-parámetros-legales-versionados--pendiente)
...
- [Sprint 62 — Corregir referencias rotas tras mover Pendientes/Preguntas-Para-Abogado/SECURITY/PDF a docs/ ✅ Completado](#sprint-62--corregir-referencias-rotas-tras-mover-pendientespreguntas-para-abogadosecuritypdf-a-docs--pendiente)
- [Sprint 63 — Documentar en README/GUIA_USUARIO las funciones de los Sprints 52-60 ✅ Completado](#sprint-63--documentar-en-readmeguia_usuario-las-funciones-de-los-sprints-52-60--pendiente)
...
## Sprint 78 — Conteo inclusivo (`+1`) en `calcular_densidad_semanas` — confirmar con el despacho ✅ Completado
...
**Cierre (2026-08-23, rutina autónoma):** el despacho confirmó (22/08/2026, ver
`Preguntas-Para-Abogado-Respondidas.md`, Sprint 78) que sí aplica el "+1" inclusivo. Implementado en
`calcular_densidad_semanas` (`app/engine/labor/ibl.py`): cada período fusionado suma `(fin - inicio).days +
1` antes de dividir por 7. El caso de prueba judicial ya citado (348 días → 50 semanas) da el mismo
resultado con el +1 (349/7 → 50) — no era una excepción real, solo coincidencia de redondeo; el caso de
períodos solapados sí cambió (6 → 7 semanas), actualizado en el test. Suite completa en verde (1530
passed). Función aislada, sigue sin conectar a ningún flujo real de liquidación (sin cambio de alcance).
...
**Contexto:** `app/engine/indexation/historical_index.py`, líneas 234-257, documenta que `_IPC_MENSUAL: dict[tuple[int, int], Decimal] = {}` está deliberadamente vacía porque no se consiguió "una tabla transcribible con confianza" (nota del 2026-08-01 en el Sprint 8 de `Pendientes.md`, líneas 802-805). Ese bloqueo ya se resolvió: **`docs/datos_publicos_fuente/ipc_mensual_dane_2003_2026.csv`** (extraído programáticamente el 2026-08-20, ver el README de esa carpeta) trae exactamente ese dato — 279 filas `anio, mes, mes_nombre, indice_ipc_base_dic2018_100`, con la base confirmada (diciembre-2018 = 100.00 exacto) y pie "Índices - Serie de empalme 2003 - 2026 / Fuente: DANE / Actualizado el 9 de Abril de 2026". Cubre enero-2003 (50.42) hasta **marzo-2026 (156.94)** — **corrección importante:** el propio archivo original fuente (`docs/Archivos de referencia abogado/Historico IPC.xlsx`, gitignoreado, no accesible desde la rutina en la nube) hizo pensar en una versión anterior de este sprint que el último dato era "Abril-2026 (Abril=149.66)"; extrayendo directo de la celda, 149.66 es en realidad **abril-2025** (encaja entre marzo-2025=148.68 y mayo-2025=150.14), no 2026. Usar el CSV como fuente de verdad, no esa cita. Abril-2026 en adelante todavía no está certificado (blanco). A diferencia de lo que pidió el despacho en su respuesta del Sprint 8 (dos bases, Dic-2008=100 y Dic-2018=100, enlazadas por un Factor de Enlace calculado por el software en el mes de traslape — `Preguntas-Para-Abogado-Respondidas.md`, Sprint 8), esta tabla **ya viene enlazada por el DANE en una sola base continua** (2018=100) — es decir, el trabajo de "Factor de Enlace" que el despacho pidió que el software hiciera ya está resuelto en el origen. No cubre 1967-2002: para esas fechas la única fuente que existe sigue siendo `_IPC_VARIACION_ANUAL` (variación % anual, ya cargada desde el Sprint 5).
...
## Sprint 82 — Cargar la serie histórica semanal de DTF (Banco de la República) como parámetro legal reutilizable ⚠️ Parcial
...
**Cierre parcial (2026-08-23, rutina autónoma):** el despacho respondió (22/08/2026, ver
`Preguntas-Para-Abogado-Respondidas.md`, Sprint 82) con la fórmula completa del interés de mora (régimen
dual DTF/1,5x IBC, Art. 195 núm. 4 Ley 1437/2011), pero sin contestar la pregunta original sobre si maneja
este tipo de casos y en qué área deberían vivir. Implementada y probada como función aislada
(`calcular_interes_dtf_condena_administrativa`, `app/engine/interest/condena_administrativa_dtf.py`, 6
tests nuevos), sin conectar a ninguna `AreaStrategy` — esa decisión de arquitectura sigue pendiente. El
"consejo" de congelar el conteo tras 3 meses de inactividad no se implementó (sugerencia de interfaz, no
regla obligatoria; requiere un dato que el modelo no captura). Suite completa en verde (1536 passed).
Pregunta de seguimiento acotada (solo la asignación de área) en `Preguntas-Para-Abogado-Abiertas.md`,
"Sprint 82 (seguimiento)".
...
**Cierre (2026-08-20, rutina autónoma):** Completado el alcance de documentación (sin cambio de
comportamiento). Discrepancia 365-compuesto (BASTIUM) vs. 366-lineal (plantillas i4/i4A del despacho)
documentada en el docstring de `app/engine/tax/moratory_interest.py`. Pregunta nueva agregada como sección
propia "Sprint 84" en `docs/Preguntas-Para-Abogado-Abiertas.md` (con su entrada en el índice). No se tocó
