# Carga de Datos Históricos (Sprint 5) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Populate `app/engine/indexation/historical_index.py` with real, PDF-verified historical series (SMLMV 1984-2026, IPC 1967-2025, IBC/Tasa de Usura 1997-07-01 to 2026-07-31 for the "Consumo y Ordinario" credit line) and expose three query functions, with no wiring into any liquidation strategy yet.

**Architecture:** Pure data module — three Python literals (`_SMLMV_POR_ANIO`, `_IPC_VARIACION_ANUAL`, `_TRAMOS_IBC_USURA`) plus three public query functions. No database, no new dependencies at runtime. The data was extracted from the source PDF using grid-aware table parsing (not linear text extraction, which scrambles this specific multi-column table) and cross-verified: zero gaps, one documented source anomaly resolved (Sept. 2017 quarterly/monthly certification overlap).

**Tech Stack:** Python 3.14, `decimal.Decimal`, pytest. No new runtime dependencies (the PDF-extraction tooling used to prepare this plan is a one-time dev-time step, not a library the app depends on).

**Design doc:** `docs/superpowers/specs/2026-07-15-carga-datos-historicos-design.md` — read it first for the full rationale, especially the "Hallazgo relevante" section explaining why this data required grid-aware extraction instead of the linear text reads used elsewhere in this project's PDF work.

**Note on task granularity:** unlike prior sprints, this one is almost entirely sequential — Task 2 appends to the exact file Task 1 creates, so there's no safe way to parallelize them across isolated worktrees. Execute in order.

---

### Task 1: SMLMV and IPC data + query functions

**Files:**
- Create: `app/engine/indexation/historical_index.py`
- Create: `tests/engine/test_historical_index.py`
...
Expected: FAIL — `ModuleNotFoundError: No module named 'app.engine.indexation.historical_index'` (the file is currently 0 bytes, not even a valid module target for these imports until created).

- [ ] **Step 3: Implement SMLMV and IPC**

Create `app/engine/indexation/historical_index.py` with this exact content:
...
Expected: all tests pass (104 previous + 8 new = 112)

- [ ] **Step 6: Commit**

```bash
git add app/engine/indexation/historical_index.py tests/engine/test_historical_index.py
git commit -m "feat(indexation): add SMLMV and IPC historical series with query functions"
```

---

### Task 2: IBC / Tasa de Usura data + query function

This appends to the file Task 1 created. **Do not start this task until Task 1 is committed** — there is no safe way to parallelize this against Task 1, since both edit the same file.

**Files:**
- Modify: `app/engine/indexation/historical_index.py` (append)
- Modify: `tests/engine/test_historical_index.py` (append)

- [ ] **Step 1: Write the failing tests**

Append to `tests/engine/test_historical_index.py`. First, replace the entire top-of-file import
section (the 4 lines Task 1 created: `from datetime import date`, `from decimal import Decimal`, blank
line, `import pytest`, blank line, and the `from app.engine.indexation.historical_index import (...)`
block) with this complete block:
...
This is the complete import section for the file after this step — it replaces Task 1's version
entirely (adds `timedelta` to the `datetime` import, adds `_TRAMOS_IBC_USURA` and
`get_ibc_usura_for_date` to the `historical_index` import). Do not leave Task 1's old import block in
place alongside this one.
...
Run: `python -m pytest tests/engine/test_historical_index.py -v`
Expected: FAIL — `ImportError: cannot import name '_TRAMOS_IBC_USURA' from 'app.engine.indexation.historical_index'` (and `get_ibc_usura_for_date` doesn't exist yet either).

- [ ] **Step 3: Implement IBC/Usura**

Append to `app/engine/indexation/historical_index.py` (after the IPC section):
...
Run: `python -m pytest -q`
Expected: all tests pass (104 previous + 14 new = 118)

- [ ] **Step 6: Commit**

```bash
git add app/engine/indexation/historical_index.py tests/engine/test_historical_index.py
git commit -m "feat(indexation): add IBC/Tasa de Usura historical series (Consumo y Ordinario, 1997-2026)"
```

---

### Task 3: Documentation — Pendientes.md, README.md, Guía de Usuario

This sprint has almost no user-visible surface (no GUI, no wiring into any strategy yet) — the documentation update is correspondingly modest, but the project's standing rule still applies: no doc may claim something the code doesn't yet do, and nothing may silently omit that a dependency other sprints were waiting on is now satisfied.
...
- [ ] **Step 1: Update `Pendientes.md`**

In the "## Sprint 5 — Carga de datos históricos..." section, find the `**Definición de Hecho:**` heading and insert a `**Estado:**` block immediately before it (same pattern used for Sprint 2):
...
In the "🚧 En desarrollo" paragraph of the "Estado actual" section, append one sentence noting the new internal dependency is satisfied. Find the paragraph (it currently ends with "...El plan completo, sprint por sprint, está en **[Pendientes.md](Pendientes.md)**.") and add a sentence right before that final link sentence:
...
This sprint doesn't change anything a user interacts with, so no new numbered section is needed. Update the one existing bullet that specifically names this dependency: in section 8 ("Funciones pendientes o en desarrollo"), find:
...
Read back the three changed passages (`Pendientes.md` Sprint 5 section, `README.md` "Estado actual", `docs/GUIA_USUARIO.md` section 8) and confirm no broken markdown, no contradictory claims, and that nothing implies this sprint added a user-facing feature (it didn't).

- [ ] **Step 5: Run the full suite** (docs-only change, should be a no-op check)

Run: `python -m pytest -q`
Expected: all tests pass (118)

- [ ] **Step 6: Commit**

```bash
git add Pendientes.md README.md docs/GUIA_USUARIO.md
git commit -m "docs: document Sprint 5 historical data series (SMLMV, IPC, IBC/Usura) and remaining UVT gap"
```
...
Run: `python -m pytest -q`
Expected: all tests pass, no failures (104 baseline + 14 new in `test_historical_index.py` = 118)

- [ ] **Step 2: Spot-check the data against the PDF one more time, independently of the extraction script**

Open `REQUERIMIENTOS DE CALCULO Y REGLAS LOGICAS - BASTIUM.pdf` pages 55-62 (or the design spec, which
references the exact pages) and manually compare at least 5 values from `historical_index.py` against
the source: SMLMV 2026, SMLMV 1984, IPC variación 2025, IBC/Usura for `2017-08-31` and `2017-09-01`
(the documented anomaly boundary). This is a second, independent human/agent check on top of the
automated tests, specifically because this data feeds real legal interest calculations.

- [ ] **Step 3: Confirm no engine wiring was accidentally introduced**

Run: `git diff main --stat` (or the equivalent against whatever base branch this was built on) and
confirm the only files touched are `app/engine/indexation/historical_index.py`,
`tests/engine/test_historical_index.py`, `Pendientes.md`, `README.md`, and `docs/GUIA_USUARIO.md`. No
file under `app/services/` or `app/views/` should appear — this sprint is explicitly data-only.
...
- UVT historical series — no complete source found in the PDF, documented as pending in `Pendientes.md`.
- Microcrédito or Crédito Popular Productivo Rural IBC/Usura lines — only "Consumo y Ordinario" is modeled, per the user's explicit choice during brainstorming.
- Any wiring of these series into `CivilFamiliaStrategy`, `ComercialStrategy`, or `IPCIndexation` — that's Sprint 8 (IPC) and a future unassigned sprint (automatic IBC lookup for Comercial, replacing today's manual `ibc_vigente_anual` field).
