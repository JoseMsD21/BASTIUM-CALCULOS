...
def test_theme_claro_define_indicador_de_groupbox():
    contenido = THEME_QSS.read_text(encoding="utf-8")
    assert "QGroupBox::indicator {" in contenido
    assert "QGroupBox::indicator:checked {" in contenido
    assert "QGroupBox::indicator:hover {" in contenido
    assert "QGroupBox::indicator:disabled {" in contenido


def test_theme_claro_indicador_groupbox_usa_colores_de_marca():
    contenido = THEME_QSS.read_text(encoding="utf-8")
    bloque_normal = contenido.split("QGroupBox::indicator {", 1)[1].split("}", 1)[0]
    assert "#D8CDBB" in bloque_normal  # borde estandar sin marcar
    bloque_checked = contenido.split("QGroupBox::indicator:checked {", 1)[1].split("}", 1)[0]
    assert "#AE1C21" in bloque_checked  # primario burdeos al marcar


def test_theme_oscuro_define_indicador_de_groupbox():
    contenido = THEME_DARK_QSS.read_text(encoding="utf-8")
    assert "QGroupBox::indicator {" in contenido
    assert "QGroupBox::indicator:checked {" in contenido
    assert "QGroupBox::indicator:hover {" in contenido
...
def test_theme_oscuro_indicador_groupbox_usa_colores_de_marca():
    contenido = THEME_DARK_QSS.read_text(encoding="utf-8")
    bloque_normal = contenido.split("QGroupBox::indicator {", 1)[1].split("}", 1)[0]
    assert "#4A4039" in bloque_normal  # borde estandar sin marcar (modo oscuro)
    bloque_checked = contenido.split("QGroupBox::indicator:checked {", 1)[1].split("}", 1)[0]
