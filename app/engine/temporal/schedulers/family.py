...
class FamilyScheduler:
    ...
    def add_monthly_obligation(
        self, amount: Decimal, concept: str, due_day: int, category: str = "CHILD_SUPPORT"
    ):
        rule = RecurringRule(amount=amount, frequency="monthly", day=due_day)
        self.schedulers.append(RecurringScheduler(rule, category, concept))

    def add_yearly_obligation(
        self, amount: Decimal, concept: str, month: int, day: int, category: str = "CHILD_SUPPORT"
    ):
        rule = RecurringRule(amount=amount, frequency="yearly", month=month, day=day)
        self.schedulers.append(RecurringScheduler(rule, category, concept))
